...
DRV_HOTPLUG	equ	0 ;Testing Mister
...
leitura:
	...
	call	LerBloco2	; chamar rotina de leitura de dados
	jr	nc, .ok
...
DEV_INFO: ; MisterVersion Only Returns Basic Information
	cp	2		; somente 1 dispositivo
	jr	c, .ok
.saicomerro:
...
; Mister : Lo definiremos como no removible, el status siempre sera no changed
...
.ok:
	...
	inc	hl
	ld	a, 0;1		; flags: dispositivo R/W removivel Mister -> R/W Not Removable
	ld	(hl), a
	inc	hl
	xor	a		; CHS = 0
	ld	(hl), a
...
;    pop ix
    ...
    ld	(ix+3), 'V' ;Mister Product Name
    ld	(ix+4), 'h' ;Mister Product Name
    ld	(ix+5), 'd' ;Mister Product Name
    ld	(ix+6), 'M' ;Mister Product Name
    ld	(ix+7), 'i' ;Mister Product Name

    ld	(ix+9),  $0D ;Mister SerialNumber
    ld	(ix+10), $0E ;Mister SerialNumber
    ld	(ix+11), $0A ;Mister SerialNumber
    ld	(ix+12), $0D ;Mister SerialNumber
        

    ld	(ix), $0 ;Mister VHD ; ID del fabricante
;	ld	a, $0	;Mister VHD ;  0 = V1, 1 = V2  ->  Recogido de OCR que a su vez viene de sd_card ( sdhc(x) ) de la FPGA
	ld	(ix+15), a	; salva informacao da versao do SD (V1 ou V2) no byte 15 do CID ----- 0 = V1
...
;	ld	(ix+15), a	; salva informacao da versao do SD (V1 ou V2) no byte 15 do CID ----- 0 = V1
    ...
    or a
	call	z, mudarTamanhoBlocoPara512	; se bit CCS do OCR for 1, eh cartao SDV2 (Block address - SDHC ou SDXD) Llama si V1
...
;   Mister VHD
	...
	ld	c, $04	 ; 0x4000 Nro de bloques de 512Bytes para el vhd de 128 Megas -> (128*1024*1024)/512 = 2621244 (0x40000)
	ld	d, $00   ; 
	ld	e, $00 
...
GravarBloco:
	...
	or	a
	call	z, blocoParaByte	; se for SDV1 coverter blocos para bytes Mister V1 pero en bloques
	call	enableSD
...
; Print information about Sd R/W
; Prints Nro Blocks and Nro Block
; BCDE = Sd Block to R/W
PrintSdOffset:
...
; Convert BC to ASCII HEX
    pop hl
    
   	ld	de, WRKAREA.DebugNumBlk
    ld	a, (WRKAREA.NUMBLOCKS)
    call HexToAscii
    

    ld  a, h
    ld	de, WRKAREA.DebugBlock 
    call HexToAscii
    
    ld  a, l
    call HexToAscii

; Convert DE to ASCII HEX    
    pop hl 
...
;.NUMSD		db 1	; Currently selected card: 1 or 2 -> 1
